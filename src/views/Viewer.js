...
class Viewer extends Component {
  ...
  close() {
    const { index, history, match } = this.props;
  ...
  previous() {
    const { history, previousPath } = this.props;
    if (previousPath) {
...
function select(store, props) {
  ...
  if (
    mediumIndex + 1 > media.length - 1 &&
    index < store.albums[params.album].pages
  ) {
