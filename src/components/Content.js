...
const createMapStateToProps = keep => {
  return (state, props) => {
...
const blockify = (component, keep) => {
  return connect(createMapStateToProps(keep), mapDispatchToProps)(component);
};
...
const mapping = {
  ...
  title: {
    type: blockify(Title, ['path', 'anchor', 'active']),
  },
  heading: {
  ...
  video: {
    type: blockify(Video, ['path', 'dispatch', 'active', 'anchor']),
  },
};
...
function generate(nodes, dispatch, index, history, i) {
  ...
  return nodes.map(node => {
    if (node.kind === 'block') {
      ...
      const data = pick(node.data, options.attributes);
      const props = {
        key: node.path,
        path: node.path,
...
class Content extends Component {
  ...
  render() {
    const { nodes, index, dispatch, history, className } = this.props;
